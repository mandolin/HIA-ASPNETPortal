...
| 当前阶段 | P23.2 最小参考数据目录已完成代码、迁移和 test SQL 验证；P23.3 附件契约和 P23.4 评论事件契约已完成，进入 P23.5 状态迁移、角色处理权与 SLA 规则设计 |
| 当前唯一下一步 | 按 `work-zone/dev/plans/W-anp-P23.4-comment-event-contract.md` 与 P23 总规划，设计 P23.5 状态迁移、角色处理权、撤回、退回重提和 SLA/超期规则；不得启动通用 BPM 引擎或让评论改变事项状态。在用户明确通知 HIA 基础抽象设计已经具备前，不启动 `W-anp-P18` 的边界 inventory、adapter/facade 或 runtime proof。 |
