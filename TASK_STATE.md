...
| 当前阶段 | `W-anp-P23.0` 已确认；P23.1 参考数据、业务参数和附件边界契约已完成，准备进入 P23.2 最小目录实现 |
| 当前唯一下一步 | 按 `work-zone/dev/plans/W-anp-P23.1-reference-data-business-parameter-contract.md` 实现 P23.2：最小参考数据目录、受控种子、类型/优先级消费者迁移和相应回归。在用户明确通知 HIA 基础抽象设计已经具备前，不启动 `W-anp-P18` 的边界 inventory、adapter/facade 或 runtime proof。 |
